...
const {ErrorEmbed, GrantEmbed} = require("../constants/embeds");

module.exports = {
  data: new SlashCommandBuilder()
    .setName("grant")
    .setDescription("Выдать роль администрации")
    .addUserOption((option) => {
      return option
        .setName("user")
        .setDescription("Выберите кому выдать роль")
        .setRequired(true)
    })
   ...
   if (target.user.bot) {
    ...
    if (adminRole === adminsRoles.Attestator.value) {
      if (!baseAdminRoleService.CheckRightsForAttestator()) {
        await interaction.reply(
          {
            embeds: [ErrorEmbed('У вас недостаточно прав для выдачи этой роли', interaction)],
            ephemeral: true
          }
        )
        return false;
      ...
      if (baseAdminRoleService.TargetHasRole(adminRole)) {
        await interaction.reply(
          {
            embeds: [ErrorEmbed('Эта роль уже есть у данного пользователя', interaction)],
            ephemeral: true
          }
        )
        return false;
      }
      if (!baseAdminRoleService.NeedAttestator()) {
        await interaction.reply(
          {
            embeds: [ErrorEmbed('Данному пользователю не нужна эта роль', interaction)],
            ephemeral: true
          }
        )
        return false;
      }
      await target.roles.add(baseAdminRoleService.adminRoleObj.value);
      await interaction.reply({
        embeds: [GrantEmbed(interaction, baseAdminRoleService.adminRoleObj.name, target)]
      })
      return true;
    }
    ...
    if (!server) {
      await interaction.reply(
        {
          embeds: [ErrorEmbed('Укажите сервер для выдачи этой роли', interaction)],
          ephemeral: true
        }
      )
      return false;
    ...
    if (serverAdminRoleService.callerAdminRolesObj.value === adminsRoles.Developers.value || serverAdminRoleService.callerAdminRolesObj.value === adminsRoles.LeadInspector.value ||
      ...
      if (serverAdminRoleService.TargetHasRole(serverAdminRoleService.serverRole.value)){
        await interaction.reply(
          {
            embeds: [ErrorEmbed('Пользователь уже привязан к этому серверу', interaction)],
            ephemeral: true
          }
        )
        return false;
      }
      await target.roles.add([serverAdminRoleService.adminRoleObj.value, serverAdminRoleService.serverRole.value]);
      await interaction.reply({
        embeds: [GrantEmbed(interaction, `${serverAdminRoleService.adminRoleObj.name} - ${serverAdminRoleService.serverRole.name}`, target)]
      })
      return true;
    }
